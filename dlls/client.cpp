...
void ExportDetails(void)
{
	...
	for (int i = 0; i < iNumDetailEnts; i++)
	{
		...
		memcpy(&pWriteData[iOffset], &pModel->mins, sizeof(vec3_t));
		iOffset += sizeof(vec3_t);
		memcpy(&pWriteData[iOffset], &pModel->maxs, sizeof(vec3_t));
		iOffset += sizeof(vec3_t);
		...
		for (int j = 0; j < pModel->nummodelsurfaces; j++)
		{
			...
			iOffset += sizeof(int);
			memcpy(&pWriteData[iOffset], &psurf[j].plane->normal, sizeof(vec3_t));
			iOffset += sizeof(vec3_t);
